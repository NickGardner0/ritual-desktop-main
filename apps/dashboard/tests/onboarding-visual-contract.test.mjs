...
test('feature tour describes Ritual devices, imports, tasks, routines, and analytics', async () => {
  ...
  assert.match(tasksSource, /max-w-\[376px\]/)
  ...
  assert.match(compactFileScannerSource, /ritual-scanner-parsed/)
  assert.match(compactFileScannerSource, /ritual-scanner-processing/)
  assert.match(compactFileScannerSource, /import \{ animate, scrambleText \} from "animejs"/)
  assert.match(compactFileScannerSource, /innerHTML: scrambleText\(\{/)
  assert.match(compactFileScannerSource, /cursor: "░▒▓█"/)
  assert.match(compactFileScannerSource, /revealRate: 50/)
  assert.match(compactFileScannerSource, /settleDuration: 250/)
  assert.doesNotMatch(compactFileScannerSource, /function scrambleLine/)
  assert.match(compactFileScannerSource, /2,418 logs imported/)
  assert.match(compactFileScannerSource, /fontFamily: "var\(--ritual-font-fk\)"/)
  assert.match(compactFileScannerSource, /mask-image: linear-gradient/)
  assert.doesNotMatch(compactFileScannerSource, /ritual-scanner-line-glow/)
  ...
  assert.match(scheduleSource, /Record demo video walkthrough/)
  assert.match(scheduleSource, /Confirm CPA appointment/)
  assert.match(scheduleSource, /10 minute morning meditation/)
  assert.match(scheduleSource, /Drink 1\/2 gallon of water/)
  assert.match(scheduleSource, /Take vitamins and supplements/)
  assert.match(scheduleSource, /MenuSurface/)
  assert.match(scheduleSource, /View by List/)
  assert.match(scheduleSource, /h-\[30px\]/)
  assert.match(scheduleSource, /rounded-full/)
  assert.match(scheduleSource, /\["All", "Health", "Productivity", "Learning", "Finances"\]/)
  assert.match(scheduleSource, /ShimmeringText/)
  assert.match(scheduleSource, /@\/components\/ui\/shimmering-text/)
  assert.match(scheduleSource, /DEMO_COMPLETE_SEQUENCE/)
  assert.match(scheduleSource, /id: "meditate"/)
  assert.match(scheduleSource, /id: "water"/)
  assert.match(scheduleSource, /id: "vitamins"/)
  assert.match(scheduleSource, /startOnView=\{false\}/)
  assert.match(scheduleSource, /shimmerColor="#111111"/)
  assert.match(scheduleSource, /ritual-shimmering-text|ShimmeringText/)
  assert.doesNotMatch(scheduleSource, /Weekly planning review/)
  assert.doesNotMatch(scheduleSource, /buy groceries/)
  assert.doesNotMatch(scheduleSource, /Read 30 pages of current book/)
  assert.doesNotMatch(scheduleSource, /Quick Capture/)
  assert.doesNotMatch(scheduleSource, /3 days ago/)
  assert.doesNotMatch(scheduleSource, /PriorityBars|AlertCircle|Flag/)
  assert.doesNotMatch(scheduleSource, /"Work"/)
  assert.doesNotMatch(scheduleSource, /aria-label="Inbox"/)
  assert.doesNotMatch(scheduleSource, /aria-label="Personal project"/)
  assert.doesNotMatch(scheduleSource, /Schedule doctors appointment/)
  ...
  assert.match(appsSource, /Daily morning exercise/)
  assert.match(appsSource, /Monthly budget review/)
  assert.match(appsSource, /On completion/)
  assert.match(appsSource, /AnimatedRoutinesDemo/)
  assert.match(appsSource, /DEMO_SEQUENCE/)
  assert.match(appsSource, /useReducedMotion/)
