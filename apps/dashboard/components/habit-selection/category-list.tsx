'use client';

import React, { useMemo } from 'react';
import {
  Brain, ChartLine, FlaskConical, Heart, Monitor, Plus,
} from 'lucide-react';
import { useDesktopCapabilities } from '@/lib/desktop-capabilities';
import {
  categoryRowClass,
  connectRowActionClass,
  connectRowActionConnectedClass,
  sectionLabelClass,
} from './constants';

export type CategoryListProps = {
  computerTrackingConnected: boolean;
  isAddingComputerHabit: boolean;
  appleWatchConnected: boolean;
  ouraConnected: boolean;
  whoopConnected: boolean;
  whoopConnecting: boolean;
  garminConnected: boolean;
  plaidConnected: boolean;
  handleCategorySelect: (category: string) => void;
  handleComputerUseConnect: () => void;
  openComputerUseSettings: () => void;
  filterQuery?: string;
  activeIndex?: number;
  onActiveIndexChange?: (index: number) => void;
  onItemsChange?: (count: number) => void;
  onRegisterSelectHandlers?: (handlers: Array<() => void>) => void;
};

type CategoryItem = {
  id: string;
  label: string;
  action: string;
  connected?: boolean;
  disabled?: boolean;
  section?: string;
  icon: React.ReactNode;
  onSelect: () => void;
};

function CategoryRow({
  item,
  active,
  onHover,
}: {
  item: CategoryItem;
  active: boolean;
  onHover: () => void;
}) {
  return (
    <button
      type="button"
      data-active={active ? 'true' : undefined}
      onClick={item.onSelect}
      onMouseEnter={onHover}
      disabled={item.disabled}
      className={categoryRowClass}
    >
      <span className="flex min-w-0 flex-1 items-center gap-3">
        <span className="flex h-5 w-5 shrink-0 items-center justify-center" aria-hidden="true">
          {item.icon}
        </span>
        <span className="truncate text-[13.5px] font-normal leading-none tracking-[-0.01em] text-[#27251E]">
          {item.label}
        </span>
      </span>
      <span className={item.connected ? connectRowActionConnectedClass : connectRowActionClass}>
        {active ? '↵' : item.action}
      </span>
    </button>
  );
}

export function CategoryList({
  computerTrackingConnected,
  isAddingComputerHabit,
  appleWatchConnected,
  ouraConnected,
  whoopConnected,
  whoopConnecting,
  garminConnected,
  plaidConnected,
  handleCategorySelect,
  handleComputerUseConnect,
  openComputerUseSettings,
  filterQuery = '',
  activeIndex = 0,
  onActiveIndexChange,
  onItemsChange,
  onRegisterSelectHandlers,
}: CategoryListProps) {
  const { isDesktop } = useDesktopCapabilities();

  const selectAppleWatch = () => {
    if (appleWatchConnected) {
      handleCategorySelect('applewatch');
      return;
    }

    alert(
      '📱 To connect your Apple Watch:\n\n' +
      '1. Download the Ritual Companion app on your iPhone\n' +
      '2. Sign in with your Ritual account\n' +
      '3. Tap "Connect" to register your device\n' +
      '4. Grant HealthKit permissions\n\n' +
      'Your Apple Watch data syncs through your iPhone.'
    );
  };

  const items = useMemo<CategoryItem[]>(() => {
    const next: CategoryItem[] = [
      {
        id: 'custom',
        label: 'Custom',
        action: 'Manual',
        section: 'Suggested',
        icon: <Plus className="h-4 w-4" strokeWidth={1.75} />,
        onSelect: () => handleCategorySelect('custom'),
      },
    ];

    if (isDesktop) {
      next.push({
        id: 'computer',
        label: 'Computer Use',
        action: computerTrackingConnected ? 'Connected' : isAddingComputerHabit ? 'Adding…' : 'Connect',
        connected: computerTrackingConnected,
        disabled: isAddingComputerHabit,
        section: 'Suggested',
        icon: <Monitor className="h-4 w-4" strokeWidth={1.75} />,
        onSelect: () => {
          if (computerTrackingConnected) {
            void openComputerUseSettings();
          } else {
            void handleComputerUseConnect();
          }
        },
      });
    }

    next.push(
      {
        id: 'screentime',
        label: 'Screen Time',
        action: 'Connect',
        section: 'Devices',
        icon: (
          <>
            <img
              src="/images/Screen_Time.svg"
              alt=""
              className="h-4 w-4 object-contain"
              onError={(event) => {
                event.currentTarget.style.display = 'none';
                const fallback = event.currentTarget.nextElementSibling as HTMLElement | null;
                if (fallback) fallback.style.display = 'block';
              }}
            />
            <svg className="hidden h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
          </>
        ),
        onSelect: () => handleCategorySelect('screentime'),
      },
      {
        id: 'applewatch',
        label: 'Apple Watch',
        action: appleWatchConnected ? 'Connected' : 'Connect',
        connected: appleWatchConnected,
        section: 'Devices',
        icon: (
          <svg className="h-4 w-4" viewBox="0 0 814 1000" fill="currentColor">
            <path d="M788.1 340.9c-5.8 4.5-108.2 62.2-108.2 190.5 0 148.4 130.3 200.9 134.2 202.2-.6 3.2-20.7 71.9-68.7 141.9-42.8 61.6-87.5 123.1-155.5 123.1s-85.5-39.5-164-39.5c-76.5 0-103.7 40.8-165.9 40.8s-105.6-57-155.5-127C46.7 790.7 0 663 0 541.8c0-194.4 126.4-297.5 250.8-297.5 66.1 0 121.2 43.4 162.7 43.4 39.5 0 101.1-46 176.3-46 28.5 0 130.9 2.6 198.3 99.2zm-234-181.5c31.1-36.9 53.1-88.1 53.1-139.3 0-7.1-.6-14.3-1.9-20.1-50.6 1.9-110.8 33.7-147.1 75.8-28.5 32.4-55.1 83.6-55.1 135.5 0 7.8 1.3 15.6 1.9 18.1 3.2.6 8.4 1.3 13.6 1.3 45.4 0 102.5-30.4 135.5-71.3z" />
          </svg>
        ),
        onSelect: selectAppleWatch,
      },
      {
        id: 'oura',
        label: 'Oura Ring',
        action: ouraConnected ? 'Connected' : 'Connect',
        connected: ouraConnected,
        section: 'Devices',
        icon: <img src="/images/oura.svg" alt="" className="h-5 w-5 object-contain" />,
        onSelect: () => handleCategorySelect('oura'),
      },
      {
        id: 'whoop',
        label: 'Whoop',
        action: whoopConnecting ? 'Connecting…' : whoopConnected ? 'Connected' : 'Connect',
        connected: whoopConnected,
        disabled: whoopConnecting,
        section: 'Devices',
        icon: <img src="/images/whoop.svg" alt="" className="h-4 object-contain" />,
        onSelect: () => handleCategorySelect('whoop'),
      },
      {
        id: 'fitbit',
        label: 'Fitbit',
        action: 'Connect',
        section: 'Devices',
        icon: <img src="/images/fitbit.svg" alt="" className="h-4 object-contain" />,
        onSelect: () => handleCategorySelect('fitbit'),
      },
      {
        id: 'garmin',
        label: 'Garmin',
        action: garminConnected ? 'Connected' : 'Connect',
        connected: garminConnected,
        section: 'Devices',
        icon: <img src="/images/garmin.svg" alt="" className="h-4 object-contain" />,
        onSelect: () => handleCategorySelect('garmin'),
      },
      {
        id: 'plaid',
        label: 'Plaid',
        action: plaidConnected ? 'Connected' : 'Connect',
        connected: plaidConnected,
        section: 'Devices',
        icon: <img src="/images/plaid-mark.svg" alt="" className="h-4 object-contain" />,
        onSelect: () => handleCategorySelect('plaid'),
      },
      {
        id: 'productivity',
        label: 'Productivity',
        action: 'Manual',
        section: 'Manual',
        icon: <ChartLine className="h-4 w-4" strokeWidth={1.85} />,
        onSelect: () => handleCategorySelect('productivity'),
      },
      {
        id: 'education',
        label: 'Learning',
        action: 'Manual',
        section: 'Manual',
        icon: <Brain className="h-4 w-4" strokeWidth={1.85} />,
        onSelect: () => handleCategorySelect('education'),
      },
      {
        id: 'fitness',
        label: 'Health',
        action: 'Manual',
        section: 'Manual',
        icon: <Heart className="h-4 w-4" strokeWidth={1.85} />,
        onSelect: () => handleCategorySelect('fitness'),
      },
      {
        id: 'experiments',
        label: 'Experiments',
        action: 'Manual',
        section: 'Manual',
        icon: <FlaskConical className="h-4 w-4" strokeWidth={1.85} />,
        onSelect: () => handleCategorySelect('experiments'),
      },
    );

    const q = filterQuery.trim().toLowerCase();
    if (!q) return next;
    return next.filter((item) => item.label.toLowerCase().includes(q) || item.action.toLowerCase().includes(q));
  }, [
    isDesktop,
    computerTrackingConnected,
    isAddingComputerHabit,
    appleWatchConnected,
    ouraConnected,
    whoopConnected,
    whoopConnecting,
    garminConnected,
    plaidConnected,
    filterQuery,
    handleCategorySelect,
    handleComputerUseConnect,
    openComputerUseSettings,
  ]);

  React.useEffect(() => {
    onItemsChange?.(items.length);
  }, [items.length, onItemsChange]);

  React.useEffect(() => {
    onRegisterSelectHandlers?.(items.map((item) => item.onSelect));
  }, [items, onRegisterSelectHandlers]);

  React.useEffect(() => {
    if (!onActiveIndexChange) return;
    if (items.length === 0) return;
    if (activeIndex >= items.length) onActiveIndexChange(items.length - 1);
  }, [activeIndex, items.length, onActiveIndexChange]);

  let lastSection: string | undefined;

  return (
    <div className="flex flex-col gap-0.5 pb-1">
      {items.length === 0 ? (
        <div className="px-2.5 py-10 text-center text-[13px] text-[rgba(39,37,30,0.45)]">
          No matches for “{filterQuery.trim()}”
        </div>
      ) : (
        items.map((item, index) => {
          const showSection = Boolean(item.section && item.section !== lastSection && !filterQuery.trim());
          if (item.section) lastSection = item.section;
          return (
            <React.Fragment key={item.id}>
              {showSection ? <div className={sectionLabelClass}>{item.section}</div> : null}
              <CategoryRow
                item={item}
                active={index === activeIndex}
                onHover={() => onActiveIndexChange?.(index)}
              />
            </React.Fragment>
          );
        })
      )}
    </div>
  );
}
