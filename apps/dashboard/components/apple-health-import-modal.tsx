  ...
  const getDateFilter = useCallback(() => {
    ...
    switch (dateRange) {
      case "30days":
        ...
        return { start: toLocalDateString(d30), end: toLocalDateString(now) };
      case "90days":
        const d90 = new Date(now);
        d90.setDate(d90.getDate() - 90);
        return { start: toLocalDateString(d90), end: toLocalDateString(now) };
      case "1year":
        const d1y = new Date(now);
        d1y.setFullYear(d1y.getFullYear() - 1);
        return { start: toLocalDateString(d1y), end: toLocalDateString(now) };
      case "custom":
