...
export function SettingsFrame({
  ...
  return (
    <div
      ...
      className={cn(
        'settings-frame relative flex overflow-hidden bg-[#fbfbfa] text-[#252525]',
        variant === 'modal'
          ? 'settings-frame-modal z-10 h-[min(552px,calc(100vh-48px))] w-[min(780px,calc(100vw-48px))] rounded-[28px] border border-black/15 shadow-[0_30px_76px_rgba(0,0,0,0.20),0_8px_24px_rgba(0,0,0,0.11)] ring-1 ring-white/80'
          : 'settings-frame-window h-screen w-screen',
      )}
    >
      <aside className="settings-frame-sidebar flex w-[200px] shrink-0 flex-col border-r border-black/[0.055] bg-[#fbfbfa] px-[18px] pb-4 pt-5">
        {showTrafficLights ? (
          <div className="mb-[18px] flex h-[14px] items-center gap-2">
            <button
              type="button"
          ...
          <div className="mb-[18px] h-[14px]" aria-hidden="true" />
        )}
          ...
          {TAB_ORDER.map((id) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'ritual-snappy-row flex h-8 w-full items-center gap-3 rounded-[8px] px-[11px] text-left text-[15px] font-medium leading-none',
                  selected
                    ? 'bg-[#0f7f86] text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.18)]'
                    : 'text-[#111]',
                )}
                style={{
                  '--ritual-snappy-row-hover': selected ? '#0f7f86' : 'rgba(0,0,0,0.045)',
      ...
      <main className="settings-frame-main min-w-0 flex-1 overflow-y-auto bg-[#fbfbfa]">
        {activeTab === 'account' ? (
          <SettingsPage title="General">
            <SettingsGroup>
              <div className="flex min-h-[64px] items-center gap-3.5 px-5 py-3">
...
function SettingsPage({
  ...
  return (
    <div className={cn('settings-frame-page mx-auto w-full max-w-[532px] pb-8 pt-[38px]', embedded && 'settings-embedded-pane')}>
      <h1 className="mb-[38px] text-[16px] font-semibold leading-none text-[#444]">{title}</h1>
...
function SettingsSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      <h2 className="mb-4 text-[15px] font-semibold leading-none text-[#2b2b2b]">{title}</h2>
  ...
  return (
    <RitualSettingsRow>
      <div className="min-w-0">
        ...
        {description ? (
          <p className="mt-1 max-w-[360px] text-[13px] leading-snug text-[#7a7a7a]">{description}</p>
        ) : null}
      </div>
