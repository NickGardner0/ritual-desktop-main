...
export function AiHabitChatForm(props: AiHabitChatFormProps) {
  ...
  return (
    <div className="w-full">
      ...
      <div
        className="relative mx-auto w-full max-w-[720px] transform-none overflow-hidden rounded-[28px] border border-[rgba(24,24,27,0.14)] bg-[var(--content-bg)] shadow-[0_1px_2px_rgba(24,24,27,0.06)] transition-colors duration-150 ease-out hover:border-[rgba(24,24,27,0.2)] focus-within:border-[rgba(24,24,27,0.26)]"
      >
        <form onSubmit={handleFormSubmit}>
          <AnimatePresence initial={false}>
            {showSuggestions && hasSuggestionContent && (
              <motion.div
                ...
                className="mx-4 max-h-[102px] overflow-y-auto border-b border-gray-200/70 py-1"
              >
          ...
          <div className="flex min-h-[56px] items-center gap-1 px-2.5 py-2 text-gray-600">
            <button
              type="button"
              className={cn(
                "flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-100 transition-colors duration-150 hover:bg-gray-200/80 hover:text-gray-900",
                isUploadingScreenshot && "text-gray-900"
              )}
              onMouseDown={(e) => e.preventDefault()}
            ...
            <div className="min-w-0 flex-1 px-1.5">
              {isListening && audioStream ? (
                <div className="flex h-8 w-full items-center justify-center">
                  <VoiceWaveform isActive={true} audioStream={audioStream} className="h-7 w-full" />
                </div>
                ...
                <textarea
                  ...
                  className="block max-h-[60px] min-h-6 w-full resize-none border-0 bg-transparent py-0 font-normal text-gray-900 outline-none placeholder:text-[#9ca3af] disabled:opacity-60 text-[16px] leading-6"
                  rows={1}
            ...
            <button
              type="button"
              className={cn(
                "flex h-8 w-8 shrink-0 items-center justify-center bg-transparent transition-colors duration-150",
            ...
            >
              {isListening ? (
                ...
                <Mic className="h-[18px] w-[18px] stroke-[1.5]" />
              )}
            </button>
            ...
            <button
              ...
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-black text-white transition-opacity duration-150 hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-35"
              aria-label="Submit"
            >
              {submitButtonLoading ? (
