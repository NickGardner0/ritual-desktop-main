...
export function DashboardClient() {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        if (dateRange?.from) {
          ...
          // If no date range is selected (All Time), fetch last 3 years
          // 36500 days (~100 years) was causing very slow loads
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      const items = arrayMove(orderedHabits, oldIndex, newIndex);
  ...
  const getHabitMetricDisplay = React.useCallback((habit: Habit, previewValue?: number | null): string => {
    ...
    if (dateRange?.from) {
      filteredLogs = filteredLogs.filter(log => {
        ...
        if (dateRange.to) {
          // Normalize range bounds to avoid time-of-day shifts excluding edge dates.
