...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to stop watcher device ${deviceId}:`, error);
      return NextResponse.json({ error: 'Failed to stop watcher device' }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error stopping watcher device:', error);
