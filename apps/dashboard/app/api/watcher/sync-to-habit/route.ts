...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${BACKEND_URL}/api/watcher/sync-to-habit${queryString ? `?${queryString}` : ""}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.detail || "Failed to sync to habit" },
    ...
    console.error("Error syncing to habit:", error);
    return NextResponse.json(
