  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return NextResponse.json({ connected: false, sync_hour: 9 });
      }

      const error = await response.text();
      console.error('Failed to fetch Whoop status:', error);
      return NextResponse.json({ error: 'Failed to fetch Whoop status' }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error fetching Whoop status:', error);
    return NextResponse.json({ connected: false, sync_hour: 9 });
  }
}
