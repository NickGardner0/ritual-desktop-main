...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_CONFIG.PYTHON_API_URL}/api/wearables/apple/export_schedule`, {
      method: 'GET',
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to save schedule' }, { status: response.status });
    }
