...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_CONFIG.PYTHON_API_URL}/api/wearables/apple/metric_catalog`, {
      method: 'GET',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to fetch metric catalog' }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error fetching metric catalog:', error);
