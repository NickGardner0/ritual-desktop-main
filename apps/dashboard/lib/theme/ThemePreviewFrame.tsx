...
/**
 * Ritual brand sidebar-gradient stop tokens for Appearance preview tiles.
 * Concrete colors are defined on :root in globals.css.
 */
export const RITUAL_GRADIENT_STOPS: Record<
  string,
  { top: string; bottom: string }
> = {
  ritual: {
    top: "var(--ritual-gradient-light-top)",
    bottom: "var(--ritual-gradient-light-bottom)",
  },
  "ritual-dark": {
    top: "var(--ritual-gradient-dark-top)",
    bottom: "var(--ritual-gradient-dark-bottom)",
  },
};
...
function ThemePreviewSvg({
  ...
  const gradientId = `${clipId}-ritual`;
...
function SystemPreferencePreviewSvg({
  ...
  const lightGradientId = `${clipBase}-ritual-light`;
  const darkGradientId = `${clipBase}-ritual-dark`;
