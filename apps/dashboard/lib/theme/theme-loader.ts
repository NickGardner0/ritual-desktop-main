 ...
 * https://github.com/block/buzz — desktop/src/shared/theme/theme-loader.ts
 */

import type { ThemeRegistrationRaw } from "shiki";

/**
 * Ritual brand light theme. Uses Ritual's current light token intent
 * (globals.css :root) rather than a Shiki bundle. Paired with ritual-dark.
 */
export const RITUAL_THEME_NAME = "ritual";

/** Ritual brand dark theme — counterpart to {@link RITUAL_THEME_NAME}. */
export const RITUAL_DARK_THEME_NAME = "ritual-dark";

/** Hex approximations of Ritual :root / .dark shadcn tokens. */
const RITUAL_LIGHT_THEME: ThemeRegistrationRaw = {
  name: "ritual",
  type: "light",
  colors: {
...
const RITUAL_DARK_THEME: ThemeRegistrationRaw = {
  name: "ritual-dark",
  type: "dark",
  colors: {
...
/**
 * Resolve a theme name to a real Shiki bundled theme when needed for
 * syntax highlighting. Ritual brand aliases are not Shiki bundles.
 */
export function resolveShikiThemeName(name: string): SyntaxThemeName {
  if (name === RITUAL_THEME_NAME) return "github-light";
  if (name === RITUAL_DARK_THEME_NAME) return "github-dark";
...
const themeImports: Record<
  Exclude<SyntaxThemeName, "ritual" | "ritual-dark">,
...
export async function loadThemeData(
  ...
  if (name === RITUAL_THEME_NAME) return RITUAL_LIGHT_THEME;
  if (name === RITUAL_DARK_THEME_NAME) return RITUAL_DARK_THEME;
