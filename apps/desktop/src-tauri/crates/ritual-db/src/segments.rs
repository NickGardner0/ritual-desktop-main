...
impl ActivitySegment {
    ...
    pub fn infer_kind(&mut self) {
        if let Some(ref domain) = self.browser_domain {
            ...
            if domain_lower.contains("youtube") || domain_lower.contains("netflix") || domain_lower.contains("twitch") {
                self.segment_kind = "media".to_string();
            } else if domain_lower.contains("slack") || domain_lower.contains("discord") || domain_lower.contains("teams") {
                self.segment_kind = "communication".to_string();
            } else if domain_lower.contains("github") || domain_lower.contains("stackoverflow") || domain_lower.contains("docs") {
            ...
            if bundle_lower.contains("xcode") || bundle_lower.contains("vscode") || bundle_lower.contains("cursor") || bundle_lower.contains("intellij") {
                self.segment_kind = "development".to_string();
            } else if bundle_lower.contains("slack") || bundle_lower.contains("discord") || bundle_lower.contains("messages") || bundle_lower.contains("mail") {
                self.segment_kind = "communication".to_string();
            } else if bundle_lower.contains("zoom") || bundle_lower.contains("meet") || bundle_lower.contains("teams") {
                self.segment_kind = "meeting".to_string();
            } else if bundle_lower.contains("spotify") || bundle_lower.contains("music") || bundle_lower.contains("vlc") {
                self.segment_kind = "media".to_string();
            } else {
...
impl<'a> SegmentOps<'a> {
    ...
    pub async fn update_segment_frame_count(&self, segment_id: i64, frame_count: i64) -> Result<()> {
        self.conn.execute(
    ...
    pub async fn link_frames_by_timestamp(&self, segment_id: i64, ts_start: i64, ts_end: i64) -> Result<i64> {
        let result = self.conn.execute(
            r#"
    ...
    pub async fn create_segments_from_events(
        ...
        let events = activity_ops.get_events_in_range(device_id, ts_start, ts_end).await?;
            ...
            if event.is_afk {
                ...
                if let Some(mut seg) = current_segment.take() {
                    ...
                    if seg.duration_ms >= MIN_SEGMENT_DURATION_MS {
                        ...
                        // Link frames
                        let frame_count = self.link_frames_by_timestamp(id, seg.ts_start, seg.ts_end).await?;
    ...
    pub async fn get_segment_stats(&self, device_id: &str, ts_start: i64, ts_end: i64) -> Result<SegmentStats> {
        let mut rows = self.conn.query(
            r#"
            SELECT 
