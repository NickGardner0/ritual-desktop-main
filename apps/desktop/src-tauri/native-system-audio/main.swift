// Adapted from open-software-network/os-june's mac-system-audio-recorder.
// MIT License, Copyright (c) 2026 Open Software Network.
...
final class SystemAudioRecorder {
    ...
    func start(checkOnly: Bool = false) throws {
        ...
        cleanupStaleAggregateDevices(named: "Ritual System Audio")
        if !checkOnly {
    ...
    private func startAudioGraph(checkOnly: Bool = false) throws {
        ...
        tapDescription.name = "Ritual System Audio"
        ...
        let description: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Ritual System Audio",
    ...
    private func acquirePowerAssertion() {
        ...
        let reason = "Ritual recording in progress" as CFString
...
private func preflightSystemAudioPermission(logURL: URL?) -> SystemAudioPermissionStatus {
    guard let preflight = loadTCCFunction("TCCAccessPreflight", as: TCCPreflightFunction.self, logURL: logURL) else {
        writeLog("TCC preflight SPI is unavailable", logURL: logURL)
        return .unknown
    }
...
private func ensureSystemAudioPermission(logURL: URL?) throws {
    guard loadTCCFunction("TCCAccessPreflight", as: TCCPreflightFunction.self, logURL: logURL) != nil else {
        writeLog("TCC preflight SPI is unavailable; continuing with CoreAudio permission behavior", logURL: logURL)
        return
    }

    let status = preflightSystemAudioPermission(logURL: logURL)
    switch status {
    case .authorized:
        return
    case .denied:
        throw "System Audio Recording permission is denied. Enable Ritual in System Settings > Privacy & Security > Screen & System Audio Recording."
    case .unknown:
        break
    }
    ...
    guard granted else {
        throw "System Audio Recording permission was not granted. Enable Ritual in System Settings > Privacy & Security > Screen & System Audio Recording."
    }
}
...
let preflightOnly = CommandLine.arguments.contains("--preflight")
...
if !checkOnly && !preflightOnly && outputPath == nil {
    writeLog("missing output argument", logURL: logPath.map { URL(fileURLWithPath: $0) })
    emitProcessStatus(["event": "error", "message": "Usage: ritual-system-audio-recorder --output /path/to/recording.wav"], statusPath: statusPath)
    exit(2)
}

let helperLogURL = logPath.map { URL(fileURLWithPath: $0) }
if preflightOnly {
    switch preflightSystemAudioPermission(logURL: helperLogURL) {
    case .authorized:
        emitProcessStatus(["event": "authorized", "message": "System audio capture is authorized."], statusPath: statusPath)
        exit(0)
    case .denied:
        emitProcessStatus(["event": "denied", "message": "System Audio Recording permission is denied."], statusPath: statusPath)
        exit(0)
    case .unknown:
        emitProcessStatus(["event": "unknown", "message": "System Audio Recording permission has not been requested."], statusPath: statusPath)
        exit(0)
    }
}
