...
macro_rules! recorder_info {
    ($($arg:tt)*) => {
        log::info!("[RECORDER] {}", format!($($arg)*))
    };
}
...
fn find_recorder_executable() -> Option<PathBuf> {
    ...
    for path in &candidates {
        if path.exists() {
            recorder_info!("📍 Found recorder at: {:?}", path);
            return Some(path.clone());
        }
    }

    recorder_info!("⚠️ Ritual Recorder not found. Tried:");
    for path in &candidates {
        recorder_info!("   - {:?}", path);
    }
...
/// Get activity database path (watcher + sync queue).
fn get_activity_database_path() -> PathBuf {
    if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(".ritual/activity.db")
    } else {
        PathBuf::from("./activity.db")
    }
}

fn ensure_database_writable(path: &PathBuf) -> Result<(), String> {
    let conn = Connection::open(path).map_err(|e| {
        format!(
            "Failed to open canonical database {}: {}",
            path.display(),
            e
        )
    })?;
    conn.execute_batch("BEGIN IMMEDIATE; ROLLBACK;")
        .map_err(|e| {
            format!(
                "Canonical database is not writable ({}): {}",
                path.display(),
                e
            )
        })?;
...
pub async fn ensure_ffmpeg_installed() -> Result<FfmpegStatus, String> {
    recorder_info!("🎬 Checking FFmpeg installation...");

    // First check if already installed
    let status = check_ffmpeg_status();
    if status.is_installed {
        recorder_info!("✅ FFmpeg already installed at: {:?}", status.path);
        return Ok(status);
    }

    recorder_info!("📥 FFmpeg not found, will be downloaded on first recorder start...");
...
pub async fn start_recorder(config: RecorderConfig) -> Result<RecorderStatus, String> {
    recorder_info!("🎬 Starting Ritual Recorder...");
    recorder_info!("   Device ID: {}", config.device_id);
    recorder_info!("   Video Quality: {}", config.video_quality);
    recorder_info!(
        "   OCR: {}",
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        recorder_info!("🧹 Cleaned up any existing recorder processes");
    }

    // Clear stored handle
    {
        let mut guard = RECORDER_PROCESS
            .lock()
    ...
    recorder_info!("📍 Using executable: {:?}", executable);

    // Get paths
    let memory_db_path = get_memory_database_path();
    ...
    ensure_database_writable(&memory_db_path)?;
    ...
    let mut args = vec![
        "--database".to_string(),
        memory_db_path.to_string_lossy().to_string(),
    ...
    let pid = child.id();
    recorder_info!("✅ Recorder started with PID: {}", pid);
...
pub fn start_recorder_sync(config: RecorderConfig) -> Result<RecorderStatus, String> {
    recorder_info!("🎬 Starting Ritual Recorder (sync)...");
    ...
    let executable = find_recorder_executable()
        .ok_or_else(|| "Ritual Recorder executable not found.".to_string())?;
...
pub async fn stop_recorder() -> Result<RecorderStatus, String> {
    recorder_info!("🛑 Stopping Ritual Recorder...");
    ...
    if let Some(mut child) = guard.take() {
        match child.kill() {
            Ok(_) => {
                recorder_info!("✅ Recorder stopped");
                let _ = child.wait();
            }
            Err(e) => {
                recorder_info!("⚠️ Failed to kill recorder: {}", e);
            }
        }
    } else {
        recorder_info!("ℹ️ No recorder process to stop");
    }
...
pub fn get_available_monitors() -> Result<Vec<MonitorInfo>, String> {
    let executable =
        find_recorder_executable().ok_or_else(|| "Ritual Recorder not found".to_string())?;
...
pub async fn get_ocr_frames(
    ...
    recorder_info!(
        "📸 get_ocr_frames called: start={}, end={}, limit={:?}",
        start_ts,
        end_ts,
        limit
    );
    ...
    if !frames_db.exists() {
        recorder_info!("⚠️ Ritual database doesn't exist: {:?}", frames_db);
        return Ok(vec![]);
    }
    ...
    let extractable = frames
        .iter()
        .filter(|f| f.video_chunk_id.is_some() && f.frame_offset.is_some())
        .count();
    recorder_info!(
        "📸 Returning {} frames ({} extractable from completed videos)",
        frames.len(),
        extractable
    );
...
pub async fn extract_frame_image(
    ...
    let (video_path, offset): (String, i64) = if let Some(fid) = frame_id {
        ...
        recorder_info!(
            "🔍 Frame lookup: id={}, chunk={}, offset={}, path={}",
            fid,
            chunk_id,
            offset,
            path
        ...
        recorder_info!(
            "🔍 Direct lookup: chunk={}, offset={}, path={}",
            chunk_id,
            offset,
            path
        );
    ...
    let cache_key = format!("{}:{}", video_path, offset);
    {
        ...
        if let Some(cached_data) = cache.get(&cache_key) {
            recorder_info!("🎯 Frame cache hit: {}", cache_key);
...
async fn extract_frame_with_ffmpeg(
    ...
    recorder_info!("📊 Video info: duration={}s, fps={}", duration, fps);
    ...
    if offset_seconds > max_offset {
        recorder_info!(
            "⚠️ Offset {}s exceeds duration {}s, clamping to {}",
            offset_seconds,
            duration,
            max_offset
        );
    ...
    let output = Command::new(&ffmpeg)
        .args([
            ...
            "-loglevel",
            "warning", // Show warnings (not just errors) for debugging
            "-ss",
            &offset_str, // Seek position (before -i for speed)
            "-i",
            video_path, // Input file
            "-vf",
            &scale_filter, // Scale + pixel format conversion
            "-frames:v",
            "1", // Extract 1 frame (newer syntax)
            "-q:v",
            "3", // JPEG quality (lower = better, was 5)
            "-f",
    ...
    if !output_path.exists() {
        ...
        recorder_info!("⚠️ First extraction failed, trying from video start...");
        if !stderr.is_empty() {
            recorder_info!("📝 FFmpeg stderr: {}", stderr);
        }
    ...
    recorder_info!(
        "📸 Extracted frame: offset={}s, size={}KB",
        offset_str,
        frame_data.len() / 1024
    );
...
pub fn clear_frame_cache() -> Result<u32, String> {
    ...
    cache.entries.clear();
    recorder_info!("🧹 Cleared {} cached frames", count);
    Ok(count)
}
...
fn save_recorder_config(config: &RecorderConfig) -> Result<(), String> {
    ...
    recorder_info!("💾 Recorder config saved for auto-start");
    Ok(())
}
...
pub fn clear_recorder_config_cmd() -> Result<(), String> {
    ...
    if config_path.exists() {
        ...
        recorder_info!("🗑️ Recorder config cleared (auto-start disabled)");
    }
    Ok(())
}
