...
pub struct ScreenCapture {
    ...
    /// Cache app name -> bundle ID to avoid repeated AppleScript calls
    #[cfg(target_os = "macos")]
    bundle_id_cache: HashMap<String, Option<String>>,
    /// Prefetched focused window from should_record to avoid duplicate window scans
    prefetched_window: Option<WindowInfo>,
}
...
impl ScreenCapture {
    ...
    pub fn capture(&mut self) -> Result<CaptureResult> {
        ...
        let focused_window = self
            .prefetched_window
            .take()
            .or_else(|| self.get_focused_window());
    ...
    fn get_focused_window(&mut self) -> Option<WindowInfo> {
        ...
        for window in windows {
            ...
            #[cfg(target_os = "macos")]
            let bundle_id = self.resolve_bundle_id(&app_name);
    ...
    pub fn should_record(&mut self) -> bool {
        ...
        self.prefetched_window = focused.clone();
    ...
    #[cfg(target_os = "macos")]
    fn resolve_bundle_id(&mut self, app_name: &str) -> Option<String> {
        if let Some(cached) = self.bundle_id_cache.get(app_name) {
            return cached.clone();
        }
        let resolved = get_bundle_id_for_app(app_name);
        self.bundle_id_cache
            .insert(app_name.to_string(), resolved.clone());
        resolved
    }
}
