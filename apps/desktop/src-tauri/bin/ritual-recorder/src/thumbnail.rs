...
impl ThumbnailGenerator {
    ...
    pub fn ensure_directory(&self) -> Result<()> {
        std::fs::create_dir_all(&self.output_dir).context("Failed to create thumbnail directory")
    }
    ...
    pub fn generate(&self, image: &DynamicImage, timestamp: i64) -> Result<ThumbnailResult> {
        ...
        let dt = chrono::DateTime::from_timestamp_millis(timestamp).unwrap_or_else(|| Utc::now());
        let filename = format!(
            "thumb_{}_{}.jpg",
        ...
        let mut encoder = image::codecs::jpeg::JpegEncoder::new_with_quality(
            std::fs::File::create(&path).context("Failed to create thumbnail file")?,
            self.quality,
        );
        encoder
    ...
    pub fn generate_bytes(&self, image: &DynamicImage) -> Result<Vec<u8>> {
        let thumbnail = image.resize(self.width, self.height, FilterType::Triangle);
    ...
    pub fn cleanup_old_thumbnails(&self, max_age_days: u64) -> Result<CleanupResult> {
        ...
        let entries =
            std::fs::read_dir(&self.output_dir).context("Failed to read thumbnail directory")?;
