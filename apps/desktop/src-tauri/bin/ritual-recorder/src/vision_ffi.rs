...
unsafe fn create_text_request(
    ...
    // Allocate and init
    let request: Option<Retained<AnyObject>> = msg_send_id![msg_send_id![cls, alloc], init];
...
unsafe fn extract_results(request: &AnyObject) -> Result<VisionOcrResult, String> {
    ...
    for i in 0..count {
        ...
        let candidate: Option<Retained<AnyObject>> =
            msg_send_id![&*candidates, objectAtIndex: 0usize];
