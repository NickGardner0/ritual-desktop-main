...
pub fn set_active_browser(bundle_id: Option<String>) {
    if let Ok(mut state) = get_browser_state().lock() {
        ...
        let is_browser_now = bundle_id.as_ref().map(|id| is_browser(id)).unwrap_or(false);
    ...
    loop {
        ...
        let (bundle_id, last_url, last_title) = {
            ...
            match &state.active_browser {
                Some(id) => (id.clone(), state.last_url.clone(), state.last_title.clone()),
                None => continue,
            }
        };
        ...
        if url_changed || title_changed {
            debug!("🌐 Browser tab change detected in {}:", bundle_id);
            if url_changed {
                debug!(
                    "   URL: {:?} -> {:?}",
