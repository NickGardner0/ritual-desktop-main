...
class ConversationQueueService:
    ...
    async def update_item(self, user_id: str, conversation_id: str, item_id: str, payload: ConversationQueueUpdate) -> Optional[ConversationQueueItemRead]:
        async with get_db_session() as session:
            ...
            if payload.status is not None:
                ...
                elif payload.status in {"completed", "canceled", "stale", "failed"}:
    ...
    async def claim_next_item(self, user_id: str, conversation_id: str, item_id: str) -> Optional[ConversationQueueRunResponse]:
        async with get_db_session() as session:
            ...
            if stale:
                item.status = "stale"
                item.error_json = json.dumps({"message": "Conversation advanced beyond this queued prompt."})
                item.completed_at = _utc_now()
                item.updated_at = _utc_now()
                await session.commit()
                return ConversationQueueRunResponse(item=self._item_to_schema(item), stale=True)
            ...
            conversation.updated_at = _utc_now()
            await session.commit()
            return ConversationQueueRunResponse(item=self._item_to_schema(item), stale=False)
