...
export function SemanticSearch({
  ...
  const isSearching = searchMode === 'semantic' ? semanticSearching : textSearching;
  const results = searchMode === 'semantic' ? semanticResults : textResults;
  const error = searchMode === 'semantic' ? semanticError : textError;
  ...
  }, [searchQuery, searchMode, startTime, endTime, semanticSearch, textSearch]);
  ...
  const handleClear = useCallback(() => {
    ...
    clearTextResults();
  }, [clearSemanticResults, clearTextResults]);

  // Convert result to OcrFrame format for parent component
  const handleResultSelect = useCallback((result: SemanticSearchResult | TextSearchResult) => {
      ...
      <CardHeader className="pb-3">
        ...
        <div className="flex gap-2 mt-3">
          ...
          <Button 
            ...
            disabled={isSearching || !searchQuery.trim() || (searchMode === 'semantic' && !embeddingInitialized)}
          >
            {isSearching ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
        ...
        {searchMode === 'semantic' && !embeddingInitialized && (
