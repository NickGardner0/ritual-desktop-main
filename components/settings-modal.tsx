...
export function SettingsModal({ isOpen, onClose, onOpen }: SettingsModalProps) {
  ...
  // Call onOpen when modal opens to close sidebar, and reset view
  useEffect(() => {
    // Only trigger when modal transitions from closed to open
    if (isOpen && !wasOpen) {
      if (onOpen) {
        onOpen();
      }
      // Reset to account view when modal opens
      setCurrentView('account');
    }
    setWasOpen(isOpen);
  }, [isOpen, wasOpen, onOpen]);
