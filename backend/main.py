...
# ================================
# AI MEMORY ENDPOINTS - User preferences and working memory
# ================================
...
class MemoryUpdate(BaseModel):
    ...
    preferred_response_style: Optional[str] = None  # concise, balanced, detailed
    preferred_units: Optional[dict] = None
    preferred_focus_habits: Optional[list] = None
...
async def get_memory(
    ...
    Creates default memory if it doesn't exist.
    """
    try:
        memory = await memory_service.get_user_memory(current_user["id"])
        return memory
    except Exception as e:
        print(f"❌ Error getting memory: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@app.patch("/api/chat/memory")
async def update_memory(
    updates: MemoryUpdate,
    current_user = Depends(get_current_user)
):
    """
    Update user's AI memory/preferences (partial update).
    """
    try:
        ...
        memory = await memory_service.update_user_memory(current_user["id"], update_dict)
        return memory
    except HTTPException:
        raise
    except Exception as e:
...
@app.patch("/api/chat/conversations/{conversation_id}/memory")
async def update_conversation_memory(
    conversation_id: str,
    overrides: MemoryUpdate,
    current_user = Depends(get_current_user)
):
    """
    Update memory overrides for a specific conversation.
    """
    try:
        # Filter out None values
        ...
        result = await memory_service.update_conversation_memory_overrides(
            conversation_id,
            current_user["id"],
            override_dict
        )
        if result is None:
            raise HTTPException(status_code=404, detail="Conversation not found")
        return {"memory_overrides": result}
    except HTTPException:
        raise
    except Exception as e:
...
@app.get("/api/chat/memory/effective")
async def get_effective_memory(
    conversation_id: Optional[str] = None,
    current_user = Depends(get_current_user)
):
    """
    Get effective memory (user memory merged with conversation overrides).
    """
    try:
        memory = await memory_service.get_effective_memory(
            current_user["id"],
            conversation_id
        )
        return memory
    except Exception as e:
        print(f"❌ Error getting effective memory: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
