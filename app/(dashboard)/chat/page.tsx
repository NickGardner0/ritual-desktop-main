...
function buildCanvasFromToolData(
  ...
  if (toolData.dailyBreakdown && Array.isArray(toolData.dailyBreakdown) && toolData.dailyBreakdown.length > 0) {
    // Get habit name from tool data if available
    const actualHabitName = toolData.dailyBreakdownHabit?.name || habitName;
    
    const dailyData = toolData.dailyBreakdown.map((item: any) => ({
      date: item.date,
      hours: item.total_hours || (item.total_duration_seconds ? item.total_duration_seconds / 3600 : 0) || item.value || item.total_amount || 0,
    }));
    
    const totalHours = Math.round(dailyData.reduce((sum: number, d: any) => sum + d.hours, 0) * 100) / 100;
    const avgPerDay = Math.round((totalHours / dailyData.length) * 100) / 100;
    
    // Also grab min/max from stats if available
    let minValue: number | undefined;
    let maxValue: number | undefined;
    if (toolData.stats && Array.isArray(toolData.stats) && toolData.stats.length > 0) {
      const stat = toolData.stats[0];
      const isDuration = stat.total_hours && stat.total_hours > 0;
      minValue = isDuration ? stat.min_hours : stat.min_amount;
      maxValue = isDuration ? stat.max_hours : stat.max_amount;
    }
  ...
  if (toolData.stats && Array.isArray(toolData.stats) && toolData.stats.length > 0) {
    ...
    return {
      type: 'stats',
      title: `${stat.habit_name || habitName} Overview`,
      habitName: stat.habit_name || habitName,
      stats: {
        daysTracked: stat.days_with_data || 0,
        totalHours: isDuration ? stat.total_hours : undefined,
        totalAmount: !isDuration ? stat.total_amount : undefined,
        avgPerDay: isDuration ? stat.avg_hours_per_day : stat.avg_amount_per_day,
        minValue: isDuration ? stat.min_hours : stat.min_amount,
        maxValue: isDuration ? stat.max_hours : stat.max_amount,
        unit: stat.unit,
      },
...
export default function ChatPage() {
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          <div className="max-w-lg w-full space-y-6">
            ...
            <form onSubmit={handleSubmit} className="relative">
              <div className="bg-white border border-gray-200 shadow-sm overflow-hidden transition-shadow hover:shadow-md focus-within:shadow-md focus-within:border-gray-300">
                <textarea
                  ref={textareaRef}
                  value={input}
  ...
  // Chat view
  return (
    <div className="h-full flex bg-[#fbfbf9] relative">
      {/* Back button - fixed left position */}
      <button
        onClick={() => router.push('/dashboard')}
        className="absolute top-16 left-8 w-9 h-9 flex items-center justify-center bg-white border border-gray-200 hover:bg-[#F3F3F3] text-gray-500 hover:text-gray-700 transition-all z-10"
      >
        <ArrowLeft className="w-4 h-4" />
      </button>
      ...
      <div className={cn(
        ...
        <div className="flex-1 overflow-y-auto">
          <div className={cn(
            "mx-auto px-8 pt-16 pb-32 transition-all duration-300",
        ...
        {/* Input */}
        <div className="sticky bottom-0 left-0 right-0 pb-6 pt-4 bg-gradient-to-t from-white via-white to-transparent">
          <div className={cn(
            "mx-auto px-8 transition-all duration-300",
            canvasData ? "max-w-2xl" : "max-w-3xl"
          )}>
            <form onSubmit={handleSubmit}>
              <div className="bg-white border border-gray-200 shadow-sm overflow-hidden transition-shadow focus-within:shadow-md focus-within:border-gray-300">
