      ...
      {isConnected ? (
        <>
          <button
            ...
            aria-checked="true"
          >
            <span className="inline-block h-5 w-5 transform rounded-full bg-white transition-transform translate-x-5 shadow-sm" />
          </button>
          {onSync && (
            <button
              onClick={onSync}
              disabled={isSyncing}
              className="px-3 py-1.5 text-sm leading-5 border border-gray-300 rounded-none hover:bg-[#F3F3F3] text-gray-900 disabled:opacity-50"
            >
              {isSyncing ? (
                <>
    ...
    return (
      <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="bg-white border border-gray-300 p-5 h-[280px] animate-pulse" />
          ))}
        </div>
      </>
    );
