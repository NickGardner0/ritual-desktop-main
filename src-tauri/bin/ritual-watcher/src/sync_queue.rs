...
impl SyncQueue {
    ...
    pub fn queue_activity_sync(&self, event_id: i64) -> Result<(), String> {
        self.db.queue_activity_sync(event_id)
    ...
    pub fn queue_activity_update(&self, event_id: i64, ts_end: u64) -> Result<(), String> {
        self.db.queue_activity_update(event_id, ts_end as i64)
            .map_err(|e| e.to_string())?;
    ...
    pub fn get_daily_rollup(&self, _date: &str, _device_id: &str) -> Result<Option<DailyRollup>, String> {
        // ritual-db handles rollups internally
        Ok(None)
    }
}
