...
impl OcrEngine {
    ...
    pub fn recognize_with_timeout(&self, image: &DynamicImage, timeout_ms: u64) -> Result<OcrResult> {
        #[cfg(target_os = "macos")]
        {
    ...
    #[cfg(target_os = "macos")]
    fn recognize_native_vision(&self, image: &DynamicImage, level: VNRequestTextRecognitionLevel) -> Result<OcrResult> {
        ...
        match vision_ffi::recognize_text(&png_data, level) {
            Ok(result) => {
                ...
                let elements: Vec<TextElement> = result.observations.iter().map(|obs| {
                    TextElement {
                        text: obs.text.clone(),
    ...
    fn recognize_apple_fallback(&self, image: &DynamicImage) -> Result<OcrResult> {
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_millis();
        let temp_path = std::env::temp_dir().join(format!("ritual_ocr_{}_{}.png", std::process::id(), timestamp));
    ...
    #[cfg(target_os = "macos")]
    fn recognize_apple_with_timeout(&self, image: &DynamicImage, _timeout_ms: u64) -> Result<OcrResult> {
