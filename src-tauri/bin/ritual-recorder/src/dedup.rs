    ...
    pub fn should_store_with_context(
        ...
        if let Some(last_time) = self.last_frame_time {
            ...
            if gap as u64 >= self.max_gap_ms {
                ...
                self.update_context(current_hash, timestamp, window_title, app_bundle_id, ocr_text);
            ...
            if self.last_window_title.as_deref() != Some(title) {
                ...
                if is_meaningful_change {
                    debug!(
                        ...
                        self.last_window_title.as_deref().map(|s| truncate_str(s, 50)),
...
mod tests {
    ...
    fn create_test_image(r: u8) -> DynamicImage {
        let img: ImageBuffer<Rgba<u8>, Vec<u8>> = ImageBuffer::from_fn(100, 100, |_, _| {
            Rgba([r, 0, 0, 255])
        });
