...
pub async fn migrate_if_needed(conn: &Connection, data_dir: &Path) -> Result<()> {
    ...
    info!("  watcher.db: {}", if has_watcher { "found" } else { "not found" });
    info!("  frames.db: {}", if has_frames { "found" } else { "not found" });
    info!("  sync_queue.db: {}", if has_sync { "found" } else { "not found" });
    ...
    if has_watcher {
        match migrate_watcher_db(conn, &watcher_db).await {
            Ok((activity_count, afk_count)) => {
                ...
                info!("  Migrated {} activity events, {} AFK events", activity_count, afk_count);
            }
            Err(e) => {
    ...
    if has_frames {
        match migrate_frames_db(conn, &frames_db).await {
            Ok((frame_count, chunk_count)) => {
                ...
                info!("  Migrated {} OCR frames, {} video chunks", frame_count, chunk_count);
            }
            Err(e) => {
    ...
    // If migration was successful, backup legacy databases
    if result.is_success() {
        ...
        warn!("Migration completed with errors: {:?}", result.errors);
    }
...
async fn is_migration_complete(conn: &Connection) -> Result<bool> {
    ...
    Ok(rows.next().await.map_err(|e| DatabaseError::Migration(e.to_string()))?.is_some())
}
...
async fn migrate_activity_events(target: &Connection, source: &SqliteConn) -> Result<i64> {
    ...
    let has_browser_url: bool = source.query_row(
...
async fn insert_activity_batch(
    target: &Connection,
    batch: &[(String, String, i64, i64, String, String, Option<String>, Option<String>, Option<i32>, i64, Option<String>, Option<String>, i64, String, i64)]
) -> Result<()> {
    for row in batch {
        target.execute(
            r#"
...
async fn migrate_heartbeat(target: &Connection, source: &SqliteConn) -> Result<()> {
    ...
    let mut stmt = source.prepare(
        "SELECT device_id, last_seen_ts FROM watcher_heartbeat"
    )?;
...
async fn migrate_video_chunks(target: &Connection, source: &SqliteConn) -> Result<i64> {
    ...
    for row_result in rows {
        ...
        target.execute(
            r#"
            INSERT INTO video_chunks (file_path, start_time, end_time, frame_count, file_size_bytes, monitor_id, storage_tier, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            "#,
            libsql::params![
                row.0.clone(),
...
async fn insert_frame_batch(
    target: &Connection,
    batch: &[(i64, Option<i64>, Option<String>, Option<String>, Option<String>, Option<String>, f64, Option<String>, Option<i64>, Option<i64>, Option<String>, String, Option<String>)]
) -> Result<()> {
    for row in batch {
        target.execute(
            r#"
...
async fn migrate_sync_queue_db(target: &Connection, source_path: &Path) -> Result<i64> {
    ...
    for row_result in rows {
        ...
        target.execute(
            "INSERT INTO sync_queue (entry_type, event_id, ts_end, status, retry_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    let has_rollup: bool = source.query_row(
