...
fn select_best_sentences(scored: &[(String, f64)], max_length: usize) -> String {
    ...
    selected.sort_by_key(|s| original_order.iter().position(|x| x == s).unwrap_or(usize::MAX));
...
pub fn calculate_text_quality(text: &str) -> f64 {
    ...
    // Combine factors
    let score = (word_len_score * 0.25 + density_score * 0.35 + length_score * 0.25 + variety_score * 0.15).min(1.0);
...
fn is_stop_word(word: &str) -> bool {
    const STOP_WORDS: &[&str] = &[
        ...
        "you", "your", "i", "my", "me", "not", "no", "yes", "all", "any", "some",
        "such", "than", "too", "very", "just", "about", "into", "over", "after",
        "before", "between", "under", "above", "below", "up", "down", "out",
        "off", "through", "during", "here", "there", "where", "when", "how",
        "what", "which", "who", "whom", "why", "if", "then", "else", "so",
        "because", "while", "although", "though", "unless", "until", "whether",
        // UI-related stop words (common in OCR)
        "file", "edit", "view", "window", "help", "menu", "new", "open", "save",
        "close", "copy", "paste", "cut", "undo", "redo", "find", "replace",
        "settings", "preferences", "options", "tools", "format", "insert",
    ];
...
fn calculate_word_stats(words: &[WordPosition]) -> HashMap<String, WordStats> {
    ...
    for wp in words {
        ...
        // Track casing
        if wp.original.chars().all(|c| c.is_uppercase() || !c.is_alphabetic()) 
           && wp.original.chars().any(|c| c.is_alphabetic()) {
            entry.uppercase_count += 1;
        } else if wp.original.chars().next().map(|c| c.is_uppercase()).unwrap_or(false)
           && wp.original.chars().skip(1).all(|c| c.is_lowercase() || !c.is_alphabetic()) {
...
fn calculate_cooccurrence(words: &[WordPosition], window_size: usize) -> HashMap<String, HashSet<String>> {
...
fn score_words(
    ...
    stats.iter().map(|(word, stat)| {
        ...
        let avg_position = stat.positions.iter().sum::<usize>() as f64 / stat.positions.len() as f64;
...
fn generate_ngrams(
    ...
    for n in 2..=max_n {
        for window in words.windows(n) {
            ...
            let phrase: String = window.iter().map(|w| w.word.as_str()).collect::<Vec<_>>().join(" ");
            
            // Score n-gram as product of individual word scores (with adjustment)
            let score: f64 = window.iter()
...
impl NerResult {
    ...
    pub fn unique_values(&self, entity_type: EntityType) -> Vec<String> {
        self.by_type.get(&entity_type)
            .map(|entities| {
                let mut values: Vec<String> = entities.iter()
...
static TIME_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b(\d{1,2}:\d{2}(?::\d{2})?(?:\s*[AP]M)?)\b").unwrap()
});

static VERSION_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b[vV]?\d+\.\d+(?:\.\d+)?(?:-[a-zA-Z0-9.]+)?\b").unwrap()
});
...
static MONEY_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"[$€£¥]\s*\d+(?:,\d{3})*(?:\.\d{2})?|\d+(?:,\d{3})*(?:\.\d{2})?\s*(?:USD|EUR|GBP|JPY)\b").unwrap()
});
...
pub fn extract_entities(text: &str) -> NerResult {
    ...
    for cap in EMAIL_REGEX.find_iter(text) {
        ...
        result.by_type.entry(EntityType::Email).or_default().push(entity);
    }
    ...
    for cap in URL_REGEX.find_iter(text) {
        ...
        result.entities.push(entity.clone());
        result.by_type.entry(EntityType::Url).or_default().push(entity);
    }
    ...
    for cap in UNIX_PATH_REGEX.captures_iter(text) {
        if let Some(m) = cap.get(1) {
            ...
            if path.contains('/') && !path.starts_with("//") {
                ...
                result.entities.push(entity.clone());
                result.by_type.entry(EntityType::FilePath).or_default().push(entity);
            }
        }
    }
    ...
    for cap in DATE_REGEX.captures_iter(text) {
        if let Some(m) = cap.get(1) {
            ...
            result.entities.push(entity.clone());
            result.by_type.entry(EntityType::Date).or_default().push(entity);
        }
    }
    ...
    for cap in TIME_REGEX.captures_iter(text) {
        if let Some(m) = cap.get(1) {
            ...
            result.entities.push(entity.clone());
            result.by_type.entry(EntityType::Time).or_default().push(entity);
        }
    }
    ...
    for cap in VERSION_REGEX.find_iter(text) {
        ...
        result.entities.push(entity.clone());
        result.by_type.entry(EntityType::Version).or_default().push(entity);
    }
    
    // Extract IP addresses
    for cap in IP_REGEX.find_iter(text) {
        // Validate IP address parts are 0-255
        let valid = cap.as_str()
            .split('.')
        ...
        if valid {
            ...
            result.entities.push(entity.clone());
            result.by_type.entry(EntityType::IpAddress).or_default().push(entity);
        }
    }
    ...
    for cap in MONEY_REGEX.find_iter(text) {
        ...
        result.entities.push(entity.clone());
        result.by_type.entry(EntityType::Money).or_default().push(entity);
    }
    ...
    for cap in GITHUB_REPO_REGEX.captures_iter(text) {
        if let Some(m) = cap.get(1) {
                ...
                && !is_common_path_segment(parts[0])
            {
                ...
                result.entities.push(entity.clone());
                result.by_type.entry(EntityType::GitHubRepo).or_default().push(entity);
            }
        }
    }
...
fn is_common_path_segment(s: &str) -> bool {
    const COMMON_SEGMENTS: &[&str] = &[
        "src", "lib", "bin", "usr", "var", "etc", "home", "tmp", "opt",
        "api", "app", "web", "dist", "build", "node_modules", "packages",
    ];
    ...
    for (lang, patterns) in LANGUAGES {
        for pattern in *patterns {
            if text_lower.contains(pattern) {
                ...
                if !result.by_type.get(&EntityType::ProgrammingLanguage)
                    .map(|v| v.iter().any(|e| e.normalized == entity.normalized))
                    .unwrap_or(false) 
                {
                    result.entities.push(entity.clone());
                    result.by_type.entry(EntityType::ProgrammingLanguage).or_default().push(entity);
                }
                break;
            }
        }
    ...
    for (framework, patterns) in FRAMEWORKS {
        for pattern in *patterns {
            if text_lower.contains(pattern) {
                ...
                // Only add if not already present
                if !result.by_type.get(&EntityType::Framework)
                    .map(|v| v.iter().any(|e| e.normalized == entity.normalized))
                    .unwrap_or(false) 
                {
                    result.entities.push(entity.clone());
                    result.by_type.entry(EntityType::Framework).or_default().push(entity);
                }
                break;
            }
        }
...
pub fn prepare_enhanced_text_v2(
    ...
    let top_keywords: Vec<String> = yake_keywords.iter()
        .take(5)
...
mod tests {
    ...
    fn test_yake_basic_extraction() {
        ...
        assert!(keyword_strings.iter().any(|k| k.contains("learning") || k.contains("neural") || k.contains("language")));
    }
    ...
    fn test_yake_scores_meaningful_words_higher() {
        ...
        assert!(keyword_strings.iter().any(|k| 
            k.contains("rust") || k.contains("memory") || k.contains("ownership") || k.contains("programming")
        ));
    }
    ...
    fn test_ner_extract_file_path_unix() {
        let text = "The config is at /Users/nick/projects/ritual/config.toml and logs at /var/log/app.log";
        let result = extract_entities(text);
    ...
    fn test_ner_extract_frameworks() {
        ...
        assert!(fw_names.contains(&"docker") || fw_names.contains(&"fastapi") || fw_names.contains(&"next.js"));
    }
    ...
    fn test_prepare_enhanced_text_v2() {
        ...
        let langs = result.entities.unique_values(EntityType::ProgrammingLanguage);
    ...
    fn test_enhanced_text_v2_includes_entities() {
        ...
        assert!(result.enhanced_text.contains("Languages:") || result.enhanced_text.contains("python"));
        
        // Should include frameworks
        assert!(result.enhanced_text.contains("Frameworks:") || result.enhanced_text.contains("django") || result.enhanced_text.contains("postgres"));
    }
}
