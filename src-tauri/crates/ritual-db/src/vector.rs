...
pub fn smart_truncate(text: &str, max_len: usize) -> String {
    ...
    // Find a good truncation point within the limit
...
impl<'a> VectorOps<'a> {
    ...
    pub async fn get_embedding(&self, frame_id: i64) -> Result<Option<OcrEmbedding>> {
        ...
        if let Some(row) = rows.next().await.map_err(|e| DatabaseError::Query(e.to_string()))? {
            let embedding_blob: Vec<u8> = row.get(2).map_err(|e| DatabaseError::Query(e.to_string()))?;
    ...
    pub async fn semantic_search(
        ...
        let sql = build_search_query(options, &embedding_blob);
        
        let query_start = std::time::Instant::now();
        let mut rows = self.conn.query(&sql, libsql::params![embedding_blob.clone(), options.limit as i64])
            .await
            .map_err(|e| DatabaseError::Query(e.to_string()))?;
        ...
        while let Some(row) = rows.next().await.map_err(|e| DatabaseError::Query(e.to_string()))? {
            ...
            if let Some(min) = options.min_relevance {
                if relevance_score < min {
                    continue;
                }
            }
            
            // Skip if text quality is too low
            if let Some(min_quality) = options.min_text_quality {
                let quality: Option<f64> = row.get(17).ok();
            ...
            if let Some(ref activity_types) = options.activity_type_filter {
                let activity_type: Option<String> = row.get(15).ok();
                if let Some(ref at) = activity_type {
                    if !activity_types.contains(at) {
                        continue;
                    }
                }
            }
...
impl<'a> VectorOps<'a> {
    ...
    pub async fn hybrid_search(
        ...
        if fts_candidates.is_empty() {
            // Fall back to pure vector search if FTS returns nothing
            info!(
                search_type = "hybrid_fallback_vector",
                fts_ms = fts_time_ms,
                fts_candidates = 0,
                "FTS returned no results, falling back to vector search"
            );
            let vector_results = self.semantic_search(query_embedding, options).await?;
            return Ok(vector_results.into_iter().map(|r| HybridSearchResult {
                frame: r.frame,
                fts_matched: false,
                vector_distance: r.distance,
                combined_score: r.relevance_score * vector_weight,
            }).collect());
        }
        
        let embedding_blob = embedding_to_blob(query_embedding);
        
        // Step 2: Get vector scores for FTS candidates
        ...
        for frame in fts_candidates {
            ...
            let distance = match self.get_frame_distance(frame_id, &embedding_blob).await {
                Ok(Some(d)) => d,
                Ok(None) => 1.0, // No embedding, maximum distance
                Err(_) => 1.0,
            };
            ...
            // Combined score: FTS matched = 1.0, weighted by fts_weight
            let combined_score = fts_weight * 1.0 + vector_weight * vector_score;
...
/// Validate and sanitize an app bundle ID to prevent SQL injection
/// 
/// App bundle IDs should only contain alphanumeric characters, dots, hyphens, and underscores.
/// Examples: "com.apple.Safari", "com.google.Chrome", "org.mozilla.firefox"
fn sanitize_app_bundle_id(app_id: &str) -> Option<String> {
    // Validate that the app ID only contains safe characters
    // Bundle IDs are typically reverse-domain notation: com.company.AppName
    let is_valid = app_id.chars().all(|c| {
        c.is_ascii_alphanumeric() || c == '.' || c == '-' || c == '_'
    });
    
    if is_valid && !app_id.is_empty() && app_id.len() <= 256 {
        Some(app_id.to_string())
    } else {
        warn!(app_id = app_id, "Invalid app bundle ID rejected");
        None
    }
}
...
/// 
/// Note: This function builds a SQL query with time range and app filters.
/// Time ranges use numeric literals (safe). App bundle IDs are validated
/// and sanitized before being included to prevent SQL injection.
fn build_search_query(options: &SearchOptions, _embedding_blob: &[u8]) -> String {
    let mut conditions = Vec::new();
    
    if let Some((start, end)) = options.time_range {
        // Numeric literals are safe from SQL injection
        conditions.push(format!("f.timestamp >= {} AND f.timestamp <= {}", start, end));
    }
    
    if let Some(ref apps) = options.app_filter {
        if !apps.is_empty() {
            // Validate and sanitize each app bundle ID
            let sanitized_apps: Vec<String> = apps.iter()
                .filter_map(|a| sanitize_app_bundle_id(a))
                .map(|a| format!("'{}'", a))  // Safe after validation
                .collect();
            
            if !sanitized_apps.is_empty() {
                conditions.push(format!("f.app_bundle_id IN ({})", sanitized_apps.join(", ")));
            }
        }
    }
...
impl EmbeddingWorker {
    ...
    pub async fn retry_failed(
        ...
        for frame_id in frame_ids {
            ...
            let current_retry = vector_ops.get_embedding(frame_id).await?
                .map(|_| 0) // Reset if we're retrying
                .unwrap_or(0);
