...
pub fn start_embedding_worker() -> Result<String, String> {
    ...
    EMBEDDING_WORKER_RUNNING.store(true, Ordering::SeqCst);
...
pub fn auto_start_embedding_worker() {
    RUNTIME.spawn(async {
        ...
        // Check if there are frames without embeddings
        let db_guard = RITUAL_DB.read().await;
        let Some(ref db) = *db_guard else {
            return;
        };
        
        match db.get_embedding_stats().await {
            Ok(stats) => {
                if stats.frames_without_embeddings > 0 {
                    println!(
                        "📊 Found {} frames without embeddings, starting worker...",
                        stats.frames_without_embeddings
                    );
                    drop(db_guard); // Release lock before starting worker
                    let _ = start_embedding_worker();
                }
            }
            Err(e) => {
                eprintln!("⚠️ Failed to check embedding stats: {}", e);
            }
        }
    });
}
