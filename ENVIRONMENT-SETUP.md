...
1. Copy the template below to `.env.local` (development) or `.env.production.local` (production)
...
The Python backend (`backend/`) needs its own environment variables. Create `backend/.env`:
