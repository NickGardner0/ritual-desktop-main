...
| 6. One scheduler | Repository done; external closeout pending | FastAPI owns a static 13-job registry. Enabled loops start independently of maintenance; 11 clock jobs use durable occurrence claims and two queue workers use atomic row claims. Trigger source is deleted, but cloud project disablement still requires direct evidence. |
...
| Fence and observe all recurring work | maintenance-gated loops + unfenced ticks | 13-job registry + occurrence claims + scheduler health | additive | parallel clock mutation path | none | `/ready` degrades for missing enabled loops; retained proactive/Whoop/Oura/Garmin/Tesla/financial adapters share the same claim functions; queue rows use atomic claims. |
...
Canonical release-branch implementation measurement (tokei 14.0.0; generated sources omitted):
  ...
  Audit-comparable total: 189,404 after the additive durable-chat, watcher-lifecycle, explicit-route, pure model-engine, and scheduler-fencing boundaries (starting ship baseline: 187,086).
...
5. Authored production LOC is 189,404 under the canonical checked-in bucket contract after the additive durable-chat, watcher-lifecycle, explicit-route, pure model-engine, and scheduler-fencing boundaries. The starting ship baseline was 187,086; the dirty-tree 183.97k and manual ~192.6k claims are not ship-branch measurements. The 180k–185k band remains unmet and does not authorize deleting live product.
