...
| Put Oura/Garmin wearable FastAPI reads on the generated client | Bearer `fetch` for authorize/disconnect/sync/sync-settings plus leftover Apple disconnect | `apiOperationWithAuth` | ~−20 | second untyped FastAPI client in legacy wearable handlers | none | Import, onboarding, privacy, and watcher settings still use catch-all cookie fetch |
...
2. Web/iOS and long-range desktop aggregates still read backend/Tinybird as explicit `synced`. Tinybird stays the analytics projection. FastAPI owns ingest and dashboard analytics reads. Overview/metrics, chat, calendar, activity logs, Tesla/Whoop, integration overview, Apple Health JSON, and Oura/Garmin now use the generated client; markdown/CSV Apple export, import, onboarding, privacy, and watcher settings still use catch-all fetch.
