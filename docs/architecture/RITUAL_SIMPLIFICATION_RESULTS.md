...
| Put Tesla and Whoop FastAPI reads on the generated client | Bearer `fetch` for status/sync/disconnect/backfill | `apiOperationWithAuth` | ~−30 | second untyped FastAPI client in Tesla/Whoop | none | Next OAuth callbacks and desktop store-code stay. Import, Apple Watch, onboarding, privacy, and watcher settings still use catch-all cookie fetch |
...
2. Web/iOS and long-range desktop aggregates still read backend/Tinybird as explicit `synced`. Tinybird stays the analytics projection. FastAPI owns ingest and dashboard analytics reads. Overview/metrics, chat, calendar, activity logs, Tesla, and Whoop now use the generated client; import, Apple Watch, onboarding, privacy, and watcher settings still use catch-all cookie fetch.
