...
2. Web/iOS and long-range desktop aggregates still read backend/Tinybird as explicit `synced`. Tinybird stays the analytics projection. FastAPI owns ingest and dashboard analytics reads. Signed-in FastAPI JSON reads/writes use the generated client, including Reports. Raw desktop activity events read `activity.db` only. Catch-all remains for markdown/CSV Apple export, multipart import/screenshot preview, and logs inline PUT (FastAPI has no update-log op).
