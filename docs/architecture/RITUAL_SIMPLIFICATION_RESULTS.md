...
2. Web/iOS and long-range desktop aggregates still read backend/Tinybird as explicit `synced`. Tinybird stays the analytics projection. FastAPI owns ingest and dashboard analytics reads. Overview/metrics, chat, calendar, activity logs, Tesla/Whoop, integration overview, Apple Health JSON, Oura/Garmin, Plaid, onboarding, home/SSO bootstrap, habit list/logs/batch, watcher settings/sync, screen-time stats, import JSON, privacy inventory/dry-run/deletion-plan, command-palette search, account delete, location pings, screenshot confirm, habit-picker status, UI preferences, prefetch, and profile now use the generated client; markdown/CSV Apple export, multipart import/screenshot preview, vault-migration/deletion execute helpers, and `/api/watcher/activity` still use catch-all fetch.
