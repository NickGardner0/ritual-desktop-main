...
2. Web/iOS and long-range desktop aggregates still read backend/Tinybird as explicit `synced`. Tinybird stays the analytics projection. FastAPI owns ingest and dashboard analytics reads. Overview/metrics, chat, calendar, activity logs, Tesla/Whoop, integration overview, and Apple Health JSON now use the generated client; markdown/CSV Apple export, import, onboarding, privacy, and watcher settings still use catch-all fetch.
