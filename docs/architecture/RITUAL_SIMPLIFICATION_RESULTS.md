...
| Put onboarding FastAPI reads on the generated client | skipCache Bearer `fetch` for bootstrap, activation, watcher device, Computer Time habit | `apiOperationWithAuth` | ~−30 | second untyped FastAPI client in onboarding | none | Home/SSO bootstrap still uses catch-all so they can force a fresh read. Import, privacy, and watcher settings still use catch-all cookie fetch |
...
2. Web/iOS and long-range desktop aggregates still read backend/Tinybird as explicit `synced`. Tinybird stays the analytics projection. FastAPI owns ingest and dashboard analytics reads. Overview/metrics, chat, calendar, activity logs, Tesla/Whoop, integration overview, Apple Health JSON, Oura/Garmin, Plaid, and onboarding now use the generated client; markdown/CSV Apple export, home/SSO bootstrap, import, privacy, and watcher settings still use catch-all fetch.
