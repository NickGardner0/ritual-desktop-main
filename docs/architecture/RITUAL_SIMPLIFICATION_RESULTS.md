...
2. Web/iOS and long-range desktop aggregates still read backend/Tinybird as explicit `synced`. Tinybird stays the analytics projection. FastAPI owns ingest and dashboard analytics reads. Overview/metrics, chat, calendar, activity-log, Tesla/Whoop, integration overview, Apple Health JSON, Oura/Garmin, Plaid, onboarding, home/SSO bootstrap, habit list/logs/batch/create/update/delete, watcher settings/sync/stats, screen-time stats, import JSON, privacy inventory/dry-run/plan/execute/migration/erasure/private-sync, command-palette search, account delete, location pings, screenshot confirm, habit-picker status, UI preferences, prefetch, profile, entity resolve, and task/routine outbox now use the generated client; markdown/CSV Apple export, multipart import/screenshot preview, and `/api/watcher/activity` still use catch-all fetch.
