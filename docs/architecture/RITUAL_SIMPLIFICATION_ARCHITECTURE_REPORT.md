...
| Remaining projections documented | Tinybird inventory. Typesense deleted. MiniSearch stays for the in-modal picker. Dashboard Tinybird reads go through FastAPI. Signed-in FastAPI JSON reads use the generated client, including privacy execute/migration/private-sync, habit mutations, watcher stats, entity resolve, and task/routine outbox. Catch-all remains for markdown/CSV Apple export, multipart import/screenshot preview, `/api/watcher/activity`, and Next-owned chat/voice/calendar/OAuth/workflow/email routes. |
