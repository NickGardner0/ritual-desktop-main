...
|---|---|
| Production build reproducible and green | Implementation SHA `bdc34ecf` passed GitHub Actions run `32568940329` (`quality` and `desktop-rust`) and deployed successfully to Vercel and Railway. |
...
| One scheduler owner | FastAPI registry has 13 jobs; maintenance no longer gates startup. Eleven clock jobs use durable occurrence claims, two queue jobs use atomic domain row claims, and schema-v2 authenticated health exposes startup/attempt/success/duration/error/lease state plus a duplicate-identity integrity query. Railway deployment `2043bae3-91b0-428b-8f47-151831e29b4f` was healthy across restart and completed distinct 10:00/11:00 UTC hourly occurrences with empty duplicate and overlap lists. Trigger.dev **code** is deleted; cloud disable remains external. |
...
| LOC remeasured | Canonical implementation total 190,952 via `npm run audit:loc` after the additive durable-chat, watcher-lifecycle, explicit-route, model-engine, scheduler, channel-auth, desktop release-correctness, and scheduler-integrity boundaries (starting ship baseline 187,086); historical 192,474, ~192.6k, and dirty-tree 183.97k claims are reconciled in `LOC_BASELINE.md`. Next BFF is 19/39 because three named non-JSON adapters replaced generic ownership. |
