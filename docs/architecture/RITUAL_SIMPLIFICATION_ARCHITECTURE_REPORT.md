...
| Remaining projections documented | Tinybird inventory. Typesense deleted. MiniSearch stays for the in-modal picker. Dashboard Tinybird reads go through FastAPI. Overview/metrics, chat, calendar, activity-log, Tesla/Whoop, and integration overview reads use the generated client. Catch-all remains for Apple Watch settings, import, onboarding, privacy, and watcher cookie reads. |
