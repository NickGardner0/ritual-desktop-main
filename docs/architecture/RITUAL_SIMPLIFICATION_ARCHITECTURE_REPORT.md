...
| Remaining projections documented | Tinybird inventory. Typesense deleted. MiniSearch stays for the in-modal picker. Dashboard Tinybird reads go through FastAPI. Overview/metrics, chat, calendar, activity-log, Tesla/Whoop, integration overview, and Apple Health JSON reads use the generated client. Catch-all remains for markdown/CSV Apple export, import, onboarding, privacy, and watcher cookie reads. |
