...
| `habit_trends` pipe | FastAPI `GET /api/analytics/habits/trends` (Next catch-all proxy) | multi-habit trend over long ranges | pipe lag | `habit_logs` | reload datasource then replay pipe | API 503/`require_tinybird` or empty series |
