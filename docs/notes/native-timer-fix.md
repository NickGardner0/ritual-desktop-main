...
3. **Rebuild Widget (if needed):**
   ```bash
   cd apps/desktop/src-tauri/native-timer
   bash build_widget.sh
   ```

## Files Modified

1. **`apps/desktop/src-tauri/native-timer/TimerWidgetApp.swift`**
...
2. **`apps/desktop/src-tauri/src/native_widget.rs`**
...
4. **`apps/dashboard/components/dashboard-layout.tsx`**
   - Added `useEffect` hook to monitor token refresh requests
   - Polls every 500ms and writes fresh token when requested

5. **Files Deleted:**
   - `apps/desktop/src-tauri/native-timer/NativeTimer.swift`
   - `apps/desktop/src-tauri/native-timer/TimerWidget.swift`
   - `apps/desktop/src-tauri/native-timer/MinimalWidget.swift`
   - `apps/desktop/src-tauri/native-timer/bridge.h`
   - `apps/desktop/src-tauri/native-timer/timer_widget.h`
